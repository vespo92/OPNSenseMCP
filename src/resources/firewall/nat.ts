...
export class NATResource {
  ...
  private async getNATConfigViaSSH(): Promise<any> {
    if (!this.sshExecutor) {
      throw new Error(
        'SSH not configured. Please set OPNSENSE_SSH_HOST, OPNSENSE_SSH_USERNAME, and OPNSENSE_SSH_PASSWORD in environment variables.'
      );
    }
  ...
  private async saveNATConfigViaSSH(config: any): Promise<void> {
    ...
    if (!batch.success) {
      throw new Error(
        `Failed to save NAT configuration: ${batch.results.map((r) => r.stderr).join('\n')}`
      );
    }
  }
   ...
   */
  async updateOutboundRule(_uuid: string, _updates: Partial<OutboundNATRule>): Promise<boolean> {
    // For SSH implementation, we would need to identify rules by description or other fields
    console.warn(
      '[NATResource] Update via SSH not fully implemented. Use delete and create instead.'
    );
    return false;
  }
   ...
   */
  async toggleOutboundRule(_uuid: string, _enabled?: string): Promise<boolean> {
   ...
   */
  async getOutboundRule(_uuid: string): Promise<OutboundNATRule | null> {
  ...
  async createPortForward(rule: PortForwardRule): Promise<{ uuid: string; success: boolean }> {
    ...
    if (!this.sshExecutor) {
      throw new Error(
        'SSH not configured. Please set OPNSENSE_SSH_HOST, OPNSENSE_SSH_USERNAME, and OPNSENSE_SSH_PASSWORD environment variables.'
      );
    }
  ...
  async createOneToOneRule(_rule: OneToOneNATRule): Promise<{ uuid: string; success: boolean }> {
  ...
  async fixDMZNAT(params?: {
    ...
    try {
      ...
      const _dmzToLan = await this.createOutboundRule({
        enabled: '1',
        interface: 'wan',
      ...
      const _lanToDmz = await this.createOutboundRule({
        enabled: '1',
        interface: 'wan',
  ...
  async cleanupDMZNATFix(): Promise<{ success: boolean; deletedCount: number }> {
    ...
    try {
      const rules = await this.listOutboundRules();
      const mcpRules = rules.filter(
        (r) => r.description?.includes('MCP auto-fix') || r.description?.includes('Quick fix')
      );
  ...
  async analyzeNATConfiguration(): Promise<{
    ...
    if (mode === 'automatic') {
      ...
      const hasInternalNAT = outboundRules.some(
        (r) =>
          !r.nonat && internalNetworks.some((net) => r.destination_net?.includes(net.split('/')[0]))
      );
    ...
    const dmzRules = outboundRules.filter(
      (r) => r.source_net?.includes('10.0.6') || r.description?.toLowerCase().includes('dmz')
    );
    ...
    if (!this.sshExecutor) {
      ...
      recommendations.push(
        'Configure SSH credentials in environment variables for full NAT management capabilities'
      );
    }
