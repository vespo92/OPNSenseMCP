...
export class SSETransportServer {
  ...
  private onConnectionCallback: ((transport: Transport) => Promise<void>) | null = null;
  ...
  async start(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(this.port, this.host, () => {
        console.log(`SSE MCP Server listening on http://${this.host}:${this.port}`);
  ...
  getTransport(): Transport {
    ...
    if (!firstConnection) {
      throw new Error('No SSE connections available. Waiting for client connection...');
    }
