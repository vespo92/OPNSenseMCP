...
import type { SSETransportServer } from './transports/SSETransportServer.js';
  ...
  private async initialize() {
    try {
      ...
      if (
        ...
        logger.info(
          'OPNsense environment variables not configured. Server will start but requires configure tool to be called.'
        );
        return false;
      }
      ...
      logger.error(
        'Failed to initialize OPNsense MCP server:',
        error instanceof Error ? error.message : 'Unknown error'
      );
      return false;
    }
  ...
  private setupHandlers() {
          ...
          inputSchema: {
            ...
            properties: {
              blockprivatenetworks: {
                type: 'string',
                description: '0 to allow private networks, 1 to block',
              },
              blockbogons: { type: 'string', description: '0 to allow bogons, 1 to block' },
              allowinterlantraffic: {
                type: 'string',
                description: '1 to enable inter-LAN traffic',
              },
              bypassstaticroutes: {
                type: 'string',
                description: '1 to bypass firewall for static routes',
              },
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              interfaceName: {
                type: 'string',
                description: 'Interface name (e.g., lan, wan, opt8)',
              },
            },
        ...
        // HAProxy ACL Management
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              value: {
                type: 'string',
                description: 'Value to match against (e.g., ".example.com" for ssl_sni_end)',
              },
            ...
            properties: {
              ...
              type: {
                ...
                enum: [
                  ...
                  'tcp-request_content_use-server',
                  'tcp-request_inspect-delay',
                  // Connection actions
                  'http-request_deny',
                  'http-request_tarpit',
                  'http-request_auth',
                  'http-request_set-var',
                  // Logging/tracking
                  'http-request_capture',
                  'http-request_track-sc',
                  // Response actions
              ...
              backend: {
                type: 'string',
                description: 'Backend name or UUID (required for use_backend)',
              },
              condition: {
                type: 'string',
                description: 'ACL condition string (e.g., "if acl_name")',
              },
              aclNames: {
                type: 'array',
                items: { type: 'string' },
      ...
      switch (uri) {
        ...
        case 'opnsense://dhcp/leases': {
          if (!this.dhcpResource) {
            throw new McpError(ErrorCode.InternalError, 'DHCP resource not initialized');
          }
        ...
        case 'opnsense://dns/blocklist': {
          if (!this.dnsBlocklistResource) {
            throw new McpError(ErrorCode.InternalError, 'DNS blocklist resource not initialized');
          }
        ...
        case 'opnsense://haproxy/backends': {
          if (!this.haproxyResource) {
            throw new McpError(ErrorCode.InternalError, 'HAProxy resource not initialized');
          }
        ...
        case 'opnsense://macros': {
          if (!this.macroRecorder) {
            throw new McpError(ErrorCode.InternalError, 'Macro recorder not initialized');
          }
        ...
        case 'opnsense://arp': {
          if (!this.arpResource) {
            throw new McpError(ErrorCode.InternalError, 'ARP resource not initialized');
          }
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (name) {
        case 'configure': {
          if (!args) {
            throw new McpError(ErrorCode.InvalidRequest, 'Configuration parameters are required');
          }
          ...
          try {
            ...
            throw new McpError(ErrorCode.InvalidRequest, `Configuration failed: ${error.message}`);
          }
        }
        ...
        case 'get_vlan': {
          ...
          if (!vlan) {
            throw new McpError(ErrorCode.InvalidRequest, `VLAN ${args.tag} not found`);
          }
          return {
            content: [
              {
        ...
        case 'get_firewall_rule': {
          await this.ensureInitialized();
          if (!args || !args.uuid) {
            throw new McpError(ErrorCode.InvalidRequest, 'uuid parameter is required');
          }
          const rule = await this.firewallRuleResource?.get(args.uuid as string);
          if (!rule) {
            throw new McpError(ErrorCode.InvalidRequest, `Firewall rule ${args.uuid} not found`);
          }
          return {
            content: [
              {
        ...
        case 'create_firewall_preset': {
          ...
          try {
            const presetRule = this.firewallRuleResource?.createPreset(args.preset as string, {
              source: args.source,
        ...
        case 'find_firewall_rules': {
          ...
          if (!args || !args.description) {
            throw new McpError(ErrorCode.InvalidRequest, 'description parameter is required');
          }

          try {
            const rules = await this.firewallRuleResource?.findByDescription(
        ...
        case 'create_backup': {
          ...
          if (!this.backupManager) {
            throw new McpError(ErrorCode.InternalError, 'Backup manager not enabled.');
          }
        ...
        case 'restore_backup': {
          ...
          if (!args || !args.backupId) {
            throw new McpError(ErrorCode.InvalidRequest, 'backupId parameter is required');
          }
        ...
        case 'find_device_by_name': {
          ...
          if (!args || !args.pattern) {
            throw new McpError(ErrorCode.InvalidRequest, 'pattern parameter is required');
          }
          ...
          try {
            ...
            const formatted = results.map((lease) => this.dhcpResource?.formatLease(lease));
        ...
        case 'find_device_by_mac': {
          ...
          if (!args || !args.mac) {
            throw new McpError(ErrorCode.InvalidRequest, 'mac parameter is required');
          }
        ...
        case 'find_arp_by_ip': {
          ...
          if (!args || !args.ipPattern) {
            throw new McpError(ErrorCode.InvalidRequest, 'ipPattern parameter is required');
          }
              ...
              content: [
                {
                  type: 'text',
                  text:
                    `Found ${results.length} ARP entries:\n\n` +
        ...
        case 'find_arp_by_mac': {
          ...
          if (!args || !args.macPattern) {
            throw new McpError(ErrorCode.InvalidRequest, 'macPattern parameter is required');
          }
        ...
        case 'find_arp_by_interface': {
          ...
          if (!args || !args.interface) {
            throw new McpError(ErrorCode.InvalidRequest, 'interface parameter is required');
          }
              ...
              content: [
                {
                  type: 'text',
                  text:
                    `Found ${results.length} ARP entries on ${args.interface}:\n\n` +
                    results.map((entry) => this.arpResource?.formatEntry(entry)).join('\n'),
                },
              ],
            };
        ...
        case 'find_devices_on_vlan': {
          ...
          if (!args || !args.vlanTag) {
            throw new McpError(ErrorCode.InvalidRequest, 'vlanTag parameter is required');
          }
              ...
              content: [
                {
                  type: 'text',
                  text:
                    `Found ${results.length} devices on VLAN ${args.vlanTag}:\n\n` +
                    results.map((entry) => this.arpResource?.formatEntry(entry)).join('\n'),
                },
              ],
            };
        ...
        case 'block_domain': {
          ...
          if (!args || !args.domain) {
            throw new McpError(ErrorCode.InvalidRequest, 'domain parameter is required');
          }
        ...
        case 'block_multiple_domains': {
          ...
          if (!args || !args.domains || !Array.isArray(args.domains)) {
            throw new McpError(ErrorCode.InvalidRequest, 'domains array parameter is required');
          }
              ...
              content: [
                {
                  ...
                  text:
                    ...
                    (result.failed.length > 0
                      ? `Failed to block: ${result.failed.join(', ')}`
                      : ''),
                },
              ],
            };
        ...
        case 'apply_blocklist_category': {
          ...
          if (!args || !args.category) {
            throw new McpError(ErrorCode.InvalidRequest, 'category parameter is required');
          }
              ...
              content: [
                {
                  type: 'text',
                  text:
                    `Applied ${args.category} blocklist category.\n` +
                    `Blocked ${result.blocked.length} domains successfully.\n` +
                    (result.failed.length > 0
                      ? `Failed to block: ${result.failed.join(', ')}`
                      : ''),
        ...
        case 'search_dns_blocklist': {
          ...
          try {
            const results = await this.dnsBlocklistResource?.searchBlocklist(
              ...
              content: [
                {
                  type: 'text',
                  text:
                    `Found ${results.length} blocklist entries:\n\n` +
                    results
                      .map(
                        (entry) =>
        ...
        case 'haproxy_service_control': {
          ...
          if (!args || !args.action) {
            throw new McpError(ErrorCode.InvalidRequest, 'action parameter is required');
          }
        ...
        case 'system_update_firewall_settings': {
          ...
          try {
            const settings: any = {};
            if (args?.blockprivatenetworks !== undefined)
              settings.blockprivatenetworks = args.blockprivatenetworks;
            if (args?.blockbogons !== undefined) settings.blockbogons = args.blockbogons;
            if (args?.allowinterlantraffic !== undefined)
              settings.allowinterlantraffic = args.allowinterlantraffic;
            if (args?.bypassstaticroutes !== undefined)
              settings.bypassstaticroutes = args.bypassstaticroutes;
        ...
        case 'interface_list_overview': {
          ...
          try {
            ...
            const formatted = interfaces.map(
              (iface) =>
        ...
        case 'interface_get_config': {
          ...
          if (!args || !args.interfaceName) {
            throw new McpError(ErrorCode.InvalidRequest, 'interfaceName parameter is required');
          }

          try {
            const config = await this.interfaceConfigResource?.getInterfaceConfig(
        ...
        case 'interface_enable_intervlan_all': {
          ...
          try {
            const result =
              await this.interfaceConfigResource?.enableInterVLANRoutingOnAllInterfaces();

            const summary = result.details.map(
              (d) => `${d.interface}: ${d.success ? '✓' : '✗'} ${d.message}`
            );
        ...
        case 'interface_configure_dmz': {
          ...
          try {
            const success = await this.interfaceConfigResource?.configureDMZInterface(
              dmzInterface as string
            );
        ...
        case 'routing_fix_all': {
          ...
          try {
            ...
            throw new McpError(ErrorCode.InternalError, `Routing fix failed: ${error.message}`);
          }
        }
        ...
        case 'routing_fix_dmz': {
          ...
          try {
            ...
            throw new McpError(ErrorCode.InternalError, `DMZ routing fix failed: ${error.message}`);
          }
        }
        ...
        case 'cli_execute': {
          ...
          if (!args || !args.command) {
            throw new McpError(ErrorCode.InvalidRequest, 'command parameter is required');
          }
        ...
        case 'ssh_execute': {
          ...
          try {
            const result = await this.sshExecutor?.execute(args.command as string, {
        ...
        case 'ssh_fix_interface_blocking': {
          ...
          try {
            const result = await this.sshExecutor?.fixInterfaceBlocking(args.interface as string);
        ...
        case 'ssh_reload_firewall': {
          ...
          try {
            const result = await this.sshExecutor?.execute(
              'configctl filter reload && pfctl -f /tmp/rules.debug'
            );
        ...
        case 'ssh_backup_config': {
          ...
          try {
            const result = await this.sshExecutor?.backupConfig(args?.backupName as string);
        ...
        case 'ssh_restore_config': {
          ...
          if (!args || !args.backupPath) {
            throw new McpError(ErrorCode.InvalidRequest, 'backupPath parameter is required');
          }

          try {
            const result = await this.sshExecutor?.restoreConfig(args.backupPath as string);
        ...
        case 'ssh_check_nfs_connectivity': {
          ...
          try {
            const result = await this.sshExecutor?.checkNFSConnectivity(args?.targetIP as string);
        ...
        case 'ssh_batch_execute': {
          ...
          if (!args || !args.commands || !Array.isArray(args.commands)) {
            throw new McpError(ErrorCode.InvalidRequest, 'commands array parameter is required');
          }

          try {
            const result = await this.sshExecutor?.executeBatch(args.commands as string[], {
        ...
        case 'haproxy_certificate_create': {
          ...
          if (!args || !args.name || !args.type) {
            throw new McpError(ErrorCode.InvalidRequest, 'name and type parameters are required');
          }
        ...
        case 'haproxy_acl_create': {
          ...
          try {
            const result = await this.haproxyResource?.addACLToFrontend(args.frontend as string, {
              name: args.name as string,
        ...
        case 'haproxy_backend_health': {
          ...
          if (!args || !args.backend) {
            throw new McpError(ErrorCode.InvalidRequest, 'backend parameter is required');
          }
        ...
        case 'macro_play': {
          ...
          if (!args || !args.id) {
            throw new McpError(ErrorCode.InvalidRequest, 'id parameter is required');
          }
        ...
        case 'macro_export': {
          ...
          if (!args || !args.path) {
            throw new McpError(ErrorCode.InvalidRequest, 'path parameter is required');
          }
        ...
        case 'nat_get_mode': {
          ...
          try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to get NAT mode: ${error.message}`);
          }
        }
        ...
        case 'nat_set_mode': {
          ...
          if (!args || !args.mode) {
            throw new McpError(ErrorCode.InvalidRequest, 'mode parameter is required');
          }
          ...
          try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to set NAT mode: ${error.message}`);
          }
        }
        ...
        case 'nat_fix_dmz': {
          ...
          try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to fix DMZ NAT: ${error.message}`);
          }
        }
...
${analysis.recommendations.length > 0 ? analysis.recommendations.map((r) => `- ${r}`).join('\n') : '- No recommendations'}`,
  ...
  async start() {
    ...
    const transportOrServer = await TransportManager.createTransport(
      transportType,
      transportOptions
    );
  ...
  private async handleIaCResourceRead(uri: string) {
    if (!this.iacEnabled) {
      throw new McpError(ErrorCode.InvalidRequest, 'IaC features are disabled');
    }
    ...
    switch (uri) {
      ...
      default:
        throw new McpError(ErrorCode.InvalidRequest, `Unknown IaC resource: ${uri}`);
    }
  }

  private async handleIaCTool(name: string, args: any) {
    if (!this.planner || !this.engine || !this.stateStore) {
      throw new McpError(ErrorCode.InternalError, 'IaC components not initialized');
    }
    ...
    switch (name) {
      ...
      default:
        throw new McpError(ErrorCode.InvalidRequest, `Unknown IaC tool: ${name}`);
    }
  }
  ...
  private async planDeployment(args: any) {
    ...
    // Create plan
    const plan = await this.planner?.planDeployment(args.name, resources, currentResourcesArray);
  ...
  private async applyDeployment(args: any) {
    ...
    if (!plan) {
      throw new McpError(ErrorCode.InvalidRequest, `Plan not found: ${args.planId}`);
    }
  ...
  private async destroyDeployment(args: any) {
    ...
    if (!deployment) {
      throw new McpError(ErrorCode.InvalidRequest, `Deployment not found: ${args.deploymentId}`);
    }

    // Create destruction plan
    const resourcesArray = Object.values(deployment.resources);
    const plan = await this.planner?.planDestruction(args.deploymentId, resourcesArray);
  ...
  private async listResourceTypes(args: any) {
    const types = resourceRegistry.listResourceTypes();
    const filtered = args.category ? types.filter((t) => t.startsWith(args.category)) : types;
