...
import { deviceFingerprints, devices, dhcpLeases, hostnamePatterns } from './schema';
...
export class DeviceFingerprintingService {
  ...
  async updateFromOuiDatabase(
    ...
    for (const entry of ouiData) {
      ...
      if (
        vendor.includes('nintendo') ||
        vendor.includes('sony computer') ||
    ...
    // Batch insert
    if (batch.length > 0) {
      await this.db.insert(deviceFingerprints).values(batch).onConflictDoNothing();
    }
  }
}
