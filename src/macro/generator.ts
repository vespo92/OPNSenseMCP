import type { APICall, MacroParameter, MacroRecording, ToolDefinition } from './types.js';
...
import { OPNSenseAPIClient } from './api/client.js';`;
  ...
  private generateToolHandler(recording: MacroRecording, toolName: string): string {
    ...
    const _paramTypeName = hasParams ? `${this.toPascalCase(recording.name)}Params` : 'any';
  ...
  private substitutePathParameters(path: string, parameters: MacroParameter[]): string {
    ...
    for (const param of parameters) {
      ...
      if (substitutedPath.includes(placeholder)) {
        substitutedPath = substitutedPath.replace(placeholder, `\${args.${param.name}}`);
      }
    }
    ...
    for (const param of parameters) {
      ...
      if (payloadStr.includes(placeholder)) {
        ...
        if (param.type !== 'string') {
          payloadStr = payloadStr.replace(placeholder, `args.${param.name}`);
        } else {
          payloadStr = payloadStr.replace(placeholder, `args.${param.name} as string`);
        }
      }
    }
