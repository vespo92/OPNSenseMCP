...
export class MacroRecorder implements IMacroRecorder {
  ...
  private substituteParameters(
    ...
    let match: RegExpExecArray | null = templateRegex.exec(callStr);
    ...
    while (match !== null) {
      ...
      match = templateRegex.exec(callStr);
    }
   ...
   */
  compareMacros(
    macro1: MacroRecording,
    macro2: MacroRecording
  ): {
    added: APICall[];
    removed: APICall[];
   ...
   */
  mergeMacros(name: string, description: string, macros: MacroRecording[]): MacroRecording {
