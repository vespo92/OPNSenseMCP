...
export class EnhancedCacheManager {
  ...
  private async initializeConnections(): Promise<void> {
    try {
      ...
      if (
        this.config.redis.cluster &&
        this.config.redis.sentinels &&
        this.config.redis.sentinels.length > 0
      ) {
  ...
  async getStatistics(): Promise<{
    ...
    const byResource = new Map(resourceStats.map((r) => [r.resource, r]));
  ...
  private async calculateSmartTTL(keyInfo: CacheKey, requestedTTL?: number): Promise<number> {
    if (requestedTTL) {
      return Math.min(
        Math.max(requestedTTL, this.config.cache?.minTTL || 60),
        this.config.cache?.maxTTL || 3600
      );
    }

    // Check if we have pattern data
    const pattern = this.patternCache.get(keyInfo.pattern);
    if (pattern?.suggestedTtl) {
  ...
  private generateRecommendations(stats: any): string[] {
    ...
    if (stats.avgResponseTime > 1000) {
      recommendations.push(
        'High average response time detected. Consider pre-warming cache for critical paths'
      );
    }

    // Analyze patterns for optimization opportunities
    const highFreqSlowPatterns = stats.patterns.filter(
      (p: any) => p.frequency > 50 && p.avgExecutionTime > 500
    );

    if (highFreqSlowPatterns.length > 0) {
      recommendations.push(
        `Optimize caching for slow, frequent queries: ${highFreqSlowPatterns.map((p: any) => p.pattern).join(', ')}`
      );
    }
  ...
  private calculateOptimalTTL(pattern: any): number {
    ...
    return Math.min(
      Math.max(calculatedTTL, this.config.cache?.minTTL || 60),
      this.config.cache?.maxTTL || 3600
    );
  }
